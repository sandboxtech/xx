...
function level.set_tech_level(player)
    ...
    local index = level.get_index(player)
    if (index > 1) then
        game.print(string.format("道友 [color=#00ffff]%s[/color] 当前境界 %s", player.name, level.get_name(player, true)))
    ...
    if nauvis_count > 0 then
        ...
        game.print(string.format("道友 [color=#00ffff]%s[/color] 在[planet=nauvis]重生%d次", player.name, nauvis_count))
    ...
    if fulgora_count > 0 then
        ...
        game.print(string.format("道友 [color=#00ffff]%s[/color] 在[planet=fulgora]重生%d次", player.name, fulgora_count))
    ...
    if vulcanus_count > 0 then
        ...
        game.print(string.format("道友 [color=#00ffff]%s[/color] 在[planet=vulcanus]重生%d次", player.name, vulcanus_count))
    ...
    if gleba_count > 0 then
        ...
        game.print(string.format("道友 [color=#00ffff]%s[/color] 在[planet=gleba]重生%d次", player.name, gleba_count))
